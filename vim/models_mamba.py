...
# import mamba_ssm
# print(dir(mamba_ssm.ops.triton))
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        ...
        if not self.if_bidirectional:
            for i, layer in enumerate(self.layers):
                ...
                if i in getattr(self, "out_indices", self.out_indices):
...
def build_vim_model(
    ...
    **kwargs
    ...
    """
    构建一个只返回中间多尺度特征的 VisionMamba 模型，方便与 Mask2Former 等 FPN-based 分割方法对接。

    参数:
      img_size       - 输入图像的高/宽（正方形）大小
      patch_size     - Patch 的尺寸
      stride         - Patch 的滑动步长
      in_chans       - 输入通道数，通常为 3
      embed_dim      - ViM 内部隐层维度
      depth          - Transformer 层数
      out_indices    - 要抽取的层号列表（0-based）
      if_fpn         - 是否打开 FPN 多尺度输出逻辑
      use_checkpoint - 是否在深层网络中启用 checkpoint 节省显存
      pretrained     - 预训练权重路径或 URL
      norm_layer     - 用哪个归一化层，通常用 nn.LayerNorm
      rms_norm       - 是否使用 Triton 加速版 RMSNorm
      **kwargs       - 额外透传给 VisionMamba 的参数

    返回:
      一个 nn.Module，当你调用它的 forward(x) 时，会返回一个 list：
      [feat_level2, feat_level3, feat_level4, feat_level5]，
      每个元素形状为 [B, embed_dim, H_i, W_i]。
    """
    model = VisionMamba(
        img_size=img_size,
...
if __name__ == "__main__":
    ...
    model = build_vim_model().to(device)
    model.eval()
    ...
    print("Model Output (first 5 layers):", output[0][:5])
